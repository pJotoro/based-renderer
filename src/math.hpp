...
union float2
{
    ...
    float const &operator[](size_t const index) const { return elements[index]; }

    float2(float const _x, float const _y)
    {
        x = _x;
        y = _y;
    }

    float2 const operator+=(float2 _v) const
    {
        float2 res{x + _v.x, y + _v.y};
        return res;
    }

    float2 const operator*(float s) const
    {
        float2 res{s*x, s*y};
        return res;
    }
...
union float3
{
    ...
    float3(float const _x, float const _y, float const _z)
    {
        x = _x;
        y = _y;
        z = _z;
    ...
    float3 const operator+=(float3 _v) const
    {
        float3 res{x + _v.x, y + _v.y, z + _v.z};
        return res;
    }

    float3 const operator*(float s) const
    {
        float3 res{s*x, s*y, s*z};
        return res;
    }
};
...
union float4
{
    ...
    float4(float const _x, float const _y, float const _z, float const _w)
    {
        x = _x;
        y = _y;
        z = _z;
    ...
    float4 const operator+=(float4 _v) const
    {
        float4 res{x + _v.x, y + _v.y, z + _v.z, w + _v.w};
        return res;
    }

    float4 const operator*(float s) const
    {
        float4 res{s*x, s*y, s*z, s*w};
        return res;
    }
};
...
union float2x2
{
    ...
    float2 const &operator[](size_t const index) const { return columns[index]; }

    float2x2(float const e0, float const e1, float const e2, float const e3)
    {
        elements[0][0] = e0;
        elements[0][1] = e1;
        elements[1][0] = e2;
    ...
    float2x2(float2 const c0, float2 const c1)
    {
        columns[0] = c0;
        columns[1] = c1;
    }

    float2x2(float const e)
    {
        elements[0][0] = e;
        elements[0][1] = 0.0f;
        elements[1][0] = 0.0f;
    ...
    float2x2() : float2x2(1.0f) {}

    void operator+=(float2 const _v)
    {
        columns[0] += _v;
        columns[1] += _v;
    }
...
union float3x3
{
    ...
    float3 const &operator[](size_t const index) const { return columns[index]; }

    float3x3(
        float const e0, float const e1, float const e2,
        float const e3, float const e4, float const e5,
        float const e6, float const e7, float const e8)
    {
        elements[0][0] = e0;
        elements[0][1] = e1;
        elements[0][2] = e2;
    ...
    float3x3(float3 const c0, float3 const c1, float3 const c2)
    {
        columns[0] = c0;
        columns[1] = c1;
        columns[2] = c2;
    ...
    float3x3() : float3x3(1.0f) {}

    void operator+(float3 const _v)
    {
        columns[0] += _v;
        columns[1] += _v;
        columns[2] += _v;
...
union float4x4
{
    ...
    float4 const &operator[](size_t const index) const { return columns[index]; }

    float4x4(
        float const e0, float const e1, float const e2, float const e3,
        float const e4, float const e5, float const e6, float const e7,
        float const e8, float const e9, float const e10, float const e11,
        float const e12, float const e13, float const e14, float const e15)
    {
        elements[0][0] = e0;
        elements[0][1] = e1;
        elements[0][2] = e2;
    ...
    float4x4() : float4x4(1.0f) {}

    void operator+(float4 const _v)
    {
        columns[0] += _v;
        columns[1] += _v;
        columns[2] += _v;
...
union quaternion
{
    ...
    quaternion(float3 const _xyz, float const _w)
    {
        xyz = _xyz;
        w = _w;
    }
...
static float dot(float2 const lhs, float2 const rhs)
{
    float res = lhs.x*rhs.x + lhs.y*rhs.y;
    return res;
}

static float dot(float3 const lhs, float3 const rhs)
{
    float res = lhs.x*rhs.x + lhs.y*rhs.y + lhs.z*rhs.z;
    return res;
}

static float dot(float4 const lhs, float4 const rhs)
{
    float res = lhs.x*rhs.x + lhs.y*rhs.y + lhs.z*rhs.z + lhs.w*rhs.w;
    return res;
}

static float3 cross(float3 const lhs, float3 const rhs)
{
    float3 res{
